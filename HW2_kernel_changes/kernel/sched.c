...
/* Band enqueue task at the beginning of the queue*/
static inline void enqueue_first_task(struct task_struct* p, prio_array_t* array)
{
...
pick_next_task:
	...
	/*'next' process was chosen to be run, though we check 
	the conditions to run its bass player process instead*/
	
	/* Band Priority Scheduler */
	Band* pb = next->band;
	if (pb != NULL) { // (d)
		pid_t candidate = next->pid;
		pid_t basist = pb->instruments[BASS];
		if (basist != -1 && candidate != basist) { // (c) + (b)

			task_t* basist_task = find_task_by_pid(basist);
			if (basist_task->state == TASK_RUNNING) { // (a)
				
			  /*give up time slice for basist*/
				dequeue_task(next, rq->active);
				enqueue_task(next, rq->expired);

				dequeue_task(basist_task, basist_task->array);
				enqueue_first_task(basist_task, rq->active);

				next = basist_task;
				printk("SUCCESS tasks switched \n");
			}
		} 
	}
	/* end of changes */
